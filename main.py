...
def get_medical_information(medical_condition, information_type):
    base_url = "https://api.clinicaltables.nlm.nih.gov/api/conditions/v3/search"
    params = {
        "terms": medical_condition,
        "df": "name",
        "maxList": 1
    }
    ...
    if data and "data" in data and len(data["data"]) > 0:
        condition_id = data["data"][0]["conditionId"]
        details_url = f"https://api.clinicaltables.nlm.nih.gov/api/conditions/v3/details/{condition_id}"
        response = requests.get(details_url)
        details = response.json()

        if details and "data" in details and len(details["data"]) > 0:
            if information_type == "symptoms":
                return details["data"][0]["signsAndSymptoms"]
            elif information_type == "treatments":
                return details["data"][0]["treatments"]
            else:
                return "Invalid information type. Please choose 'symptoms' or 'treatments'."
        else:
            return "No details found for the specified medical condition."
    else:
        return "Medical condition not found."
...
def scrape_information(url):
    ...
    information = soup.find('div', {'class': 'information-class'}).text
    return information


def generate_explanation(question):
    tokens = nltk.word_tokenize(question.lower())
    something = tokens[2:-1]
    something = ' '.join(something)
    url = 'https://www.howstuffworks.com/' + something

    try:
        information = scrape_information(url)
        sentences = sent_tokenize(information)
        explanation = ' '.join(sentences[:2])
        return explanation

    except:
        return "I'm sorry, I couldn't find an explanation for that."
    ...
    while True:
        ...
        if text is not None:
            ...
            if text == "play":
                ...
                get_response(person)
            elif text.startswith("what are the symptoms of"):
                medical_condition = text.replace("what are the symptoms of", "").strip()
                symptoms = get_medical_information(medical_condition, "symptoms")
                speak(f"The symptoms of {medical_condition} are: {symptoms}")
            elif text.startswith("what are the treatments for"):
                medical_condition = text.replace("what are the treatments for", "").strip()
                treatments = get_medical_information(medical_condition, "treatments")
                speak(f"The treatments for {medical_condition} are: {treatments}")
            else:
